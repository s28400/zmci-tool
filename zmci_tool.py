...
class ZeroCloudInterface:
    ...
    def __init__(self, user_name, user_pass):
        ...
        print(f'Found {len(self.units)} unit{"s" if len(self.units)>1 else ""}'
              ' associated with account: {self.user_name}')

    def get_info_by_command(self,
                            command_name,
                            unit_number,
        ...
        if response.status_code != 200:
            print(f'Error fetching information, status code '
                  '{response.status_code}. Check credentials and try again.')
            sys.exit(0)
    ...
    def scan_for_new_diagnostic_packets(self, unit_number, scan_interval_s):
        ...
        while(1):
            diagnostics_packet = z1.get_info_by_command(GET_LAST_TRANSMIT,
                                                        unit_number, 
                                                        None)
            # Check last received packet time against latest and see if new
            if previous_diagnostic_packet:
                if (previous_diagnostic_packet['datetime_actual'] != 
                    diagnostics_packet[0]['datetime_actual']):
...
if __name__ == "__main__":
    ...
    # Example usage:
    print('\nWelcome to the non-official Zero Motorcycle cloud interface tool')
    ...
    while(1):
        ...
        if command_list[command_index-1] == GET_HISTORY:
            print('Start and end date required for get history command.'
                  ' Note: 2 day max span.')
            start_date = input('Input start date for history '
                               'Format: YYYYMMDD): ')
            end_date = input('Input end date for history (Format: YYYYMMDD): ')
            args = f' -d start={start_date} -d end={end_date}'
        print(f'Command: {command_list[command_index-1]}, unit number: '
              f'{unit_number}, args: {args}')
        print(z1.get_info_by_command(command_list[command_index-1],
                                     unit_number,
                                     args))
